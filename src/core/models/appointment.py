...
class Appointment(Base):
    ...
    recurrence = Column(
        String, nullable=True, doc="RFC 5545 RRULE string for recurring events."
    )
    ms_event_data = Column(
        JSON, nullable=True, doc="Full original MS Graph event as JSON."
    )
    ...
    show_as = Column(
        String,
        nullable=True,
        doc="MS Graph showAs value (free, tentative, busy, oof, workingElsewhere, unknown)",
    )
    sensitivity = Column(
        String,
        nullable=True,
        doc="MS Graph sensitivity value (normal, personal, private, confidential)",
    )
    ...
    importance = Column(
        String, nullable=True, doc="Event importance (low, normal, high)"
    )
    reminder_minutes_before_start = Column(
        Integer, nullable=True, doc="Reminder minutes before start"
    )
    is_all_day = Column(Boolean, nullable=True, doc="All-day event flag")
    response_status = Column(
        JSON, nullable=True, doc="Response status (MS Graph format)"
    )
    series_master_id = Column(String, nullable=True, doc="Recurring event master id")
    online_meeting = Column(
        JSON, nullable=True, doc="Online meeting info (MS Graph format)"
    )
    body_content = Column(
        String, nullable=True, doc="Event body content (text or html)"
    )
    body_content_type = Column(
        String, nullable=True, doc="Body content type (text or html)"
    )
    body_preview = Column(String, nullable=True, doc="Short preview of body content")
    calendar_id = Column(
        String, nullable=False, doc="ID of the calendar this appointment belongs to"
    )
    is_archived = Column(
        Boolean,
        nullable=False,
        default=False,
        doc="Whether this appointment has been archived and is immutable",
    )

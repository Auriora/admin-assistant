...
class AuditLogHelper:
    ...
    @staticmethod
    def log_batch_operation_start(
        user_id: int, operation: str, batch_size: int, correlation_id: str
    ) -> int:
        """
    ...
    def log_batch_operation_end(
        ...
        status = (
            "success"
            if failure_count == 0
            else ("partial" if success_count > 0 else "failure")
        )
    ...
    def log_data_modification(
        ...
        audit_service.log_operation(
            ...
            resource_type=resource_type,
            resource_id=resource_id,
            details={"changes": changes, "fields_modified": list(changes.keys())},
            correlation_id=correlation_id,
        )

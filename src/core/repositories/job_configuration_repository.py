...
class JobConfigurationRepository:
    ...
    def get_by_archive_config_id(
        ...
        return (
            self.session.query(JobConfiguration)
            .filter_by(archive_configuration_id=archive_config_id)
            .all()
        )
    ...
    def get_active_by_archive_config_id(
        self, archive_config_id: int
    ) -> List[JobConfiguration]:
        """Retrieve all active JobConfigurations for a specific archive configuration."""
        return (
            self.session.query(JobConfiguration)
            .filter_by(archive_configuration_id=archive_config_id, is_active=True)
            .all()
        )

    def get_by_user_and_archive_config(
        self, user_id: int, archive_config_id: int
    ) -> Optional[JobConfiguration]:
        """Retrieve a JobConfiguration by user_id and archive_configuration_id (should be unique)."""
        return (
            self.session.query(JobConfiguration)
            .filter_by(user_id=user_id, archive_configuration_id=archive_config_id)
            .first()
        )
    ...
    def activate_by_archive_config_id(self, archive_config_id: int) -> int:
        ...
        count = (
            self.session.query(JobConfiguration)
            .filter_by(archive_configuration_id=archive_config_id, is_active=False)

...
def ms365_auth_callback():
    if "error" in request.args:
        current_app.logger.warning(
            f"Microsoft authentication failed: {request.args['error_description']}"
        )
        flash(
            "Microsoft authentication failed: " + request.args["error_description"],
            "danger",
        )
    ...
    try:
        ...
        if photo_resp.status_code == 200:
            ...
            if not content_type.startswith("image/"):
                current_app.logger.warning(
                    f"Invalid content type for profile photo: {content_type}"
                )
                ...
                if content_length > 5 * 1024 * 1024:
                    current_app.logger.warning(
                        f"Profile photo too large: {content_length} bytes"
                    )
                    user.profile_photo_url = None
                else:
                    ...
                    secure_filename = (
                        f"profile-{user.id}-{uuid.uuid4().hex[:8]}{file_extension}"
                    )

                    # Ensure directory exists and is secure
                    photo_dir = os.path.join(
                        current_app.static_folder, "assets", "img", "team"
                    )
    ...
    flash(
        "Microsoft 365 authentication successful and tokens stored securely!", "success"
    )
    return redirect(url_for("main.index"))
...
def archive_now():
    ...
    try:
        ...
        if not archive_config:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "No active archive configuration found for user.",
                    }
                ),
                400,
            )
        ...
        if not source_calendar_uri or not archive_calendar_uri:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "Archive configuration is missing calendar IDs.",
                    }
                ),
                400,
            )
        appointments = fetch_appointments(
            user, start_date, end_date, msgraph_session, logger=current_app.logger
        )
        ...
        if result.get("status") == "overlap":
            current_app.logger.warning(
                f"Manual archive found overlaps: {result['conflicts']}"
            )
...
def schedule_archive_job():
    ...
    try:
        ...
        if schedule_type not in ["daily", "weekly"]:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "Invalid schedule_type. Must be 'daily' or 'weekly'.",
                    }
                ),
                400,
            )
        ...
        if not (0 <= hour <= 23):
            return (
                jsonify(
                    {"status": "error", "message": "Hour must be between 0 and 23."}
                ),
                400,
            )
        ...
        if not (0 <= minute <= 59):
            return (
                jsonify(
                    {"status": "error", "message": "Minute must be between 0 and 59."}
                ),
                400,
            )
            ...
            if not archive_config or archive_config.user_id != user.id:
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": "Archive configuration not found or not owned by user.",
                        }
                    ),
                    400,
                )
        ...
        if schedule_type == "daily":
            result = scheduled_archive_service.update_user_schedule(
                user_id=user.id, schedule_type="daily", hour=hour, minute=minute
            )
        else:  # weekly
            result = scheduled_archive_service.update_user_schedule(
                user_id=user.id,

...
class JobConfiguration(Base):
    ...
    user_id = Column(Integer, ForeignKey('users.id'), nullable=False, index=True, doc="User this job configuration belongs to")
    archive_configuration_id = Column(Integer, ForeignKey('archive_configurations.id'), nullable=False, index=True, doc="Archive configuration this job uses")
    archive_window_days = Column(Integer, nullable=False, default=30, doc="Number of days to look back for archiving")
    schedule_type = Column(String(16), nullable=False, default='daily', doc="Type of schedule: 'daily', 'weekly', or 'manual'")
    schedule_hour = Column(Integer, nullable=False, default=23, doc="Hour to run scheduled jobs (0-23)")
    schedule_minute = Column(Integer, nullable=False, default=59, doc="Minute to run scheduled jobs (0-59)")
    schedule_day_of_week = Column(Integer, nullable=True, doc="Day of week for weekly jobs (0=Monday, 6=Sunday)")
    is_active = Column(Boolean, default=True, nullable=False, doc="Whether this job configuration is active")
    ...
    archive_configuration = relationship("ArchiveConfiguration", back_populates="job_configurations")
    ...
    def get_schedule_description(self) -> str:
        ...
        if self.schedule_type == 'daily':
            ...
            day_name = days[self.schedule_day_of_week] if self.schedule_day_of_week is not None else 'Unknown'
    ...
    def validate(self) -> None:
        ...
        if not (0 <= self.schedule_hour <= 23):
            raise ValueError(f"Invalid schedule_hour: {self.schedule_hour} (must be 0-23)")
        
        if not (0 <= self.schedule_minute <= 59):
            raise ValueError(f"Invalid schedule_minute: {self.schedule_minute} (must be 0-59)")
        ...
        if self.schedule_type == 'weekly':
            ...
            if not (0 <= self.schedule_day_of_week <= 6):
                raise ValueError(f"Invalid schedule_day_of_week: {self.schedule_day_of_week} (must be 0-6)")
        
        if self.archive_window_days <= 0:
            raise ValueError(f"Invalid archive_window_days: {self.archive_window_days} (must be > 0)")

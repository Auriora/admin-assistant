...
class EntityAssociationHelper:
    ...
    def list_by_source(self, db: Session, source_type: str, source_id: int) -> List[EntityAssociation]:
        return db.query(EntityAssociation).filter_by(source_type=source_type, source_id=source_id).all()

    def list_by_target(self, db: Session, target_type: str, target_id: int) -> List[EntityAssociation]:
        return db.query(EntityAssociation).filter_by(target_type=target_type, target_id=target_id).all()

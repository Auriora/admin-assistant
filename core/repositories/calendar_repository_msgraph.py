...
class MSGraphCalendarRepository(BaseCalendarRepository):
    ...
    def get_by_id(self, ms_calendar_id: str) -> Optional[Calendar]:
        ...
        try:
            ms_cal = (
                self.client.users.by_user_id(self.user.email)
                .calendars.by_calendar_id(ms_calendar_id)
                .get()
            )
            if ms_cal:
                return Calendar(
            ...
            raise RuntimeError(
                f"Failed to get calendar {ms_calendar_id} for user {self.user.id} via MS Graph: {e}"
            ) from e
    ...
    async def list_async(self) -> list[Calendar]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to list calendars for user {self.user.id} via MS Graph: {e}"
            )
    ...
    def update(self, calendar: Calendar) -> None:
        ...
        try:
            ...
            self.client.users.by_user_id(self.user.email).calendars.by_calendar_id(
                calendar.ms_calendar_id
            ).patch(body=ms_cal)
        except Exception as e:
            raise RuntimeError(
                f"Failed to update calendar {calendar.ms_calendar_id} for user {self.user.id} via MS Graph: {e}"
            ) from e
    ...
    def delete(self, ms_calendar_id: str) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to delete calendar {ms_calendar_id} for user {self.user.id} via MS Graph: {e}"
            ) from e

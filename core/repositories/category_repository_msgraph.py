...
class MSGraphCategoryRepository(BaseCategoryRepository):
    ...
    async def _get_by_id_async(self, category_id: str) -> Optional[Category]:
        ...
        try:
            categories = await self.client.users.by_user_id(self.user.email).outlook.master_categories.get()
            ...
            raise RuntimeError(f"Failed to get category {category_id} for user {self.user.id} via MS Graph: {e}") from e
    ...
    async def _add_async(self, category: Category) -> None:
        ...
        try:
            ...
            ms_cat.display_name = str(category.name) if hasattr(category, 'name') else None
            ms_cat.color = "preset0"  # Default color
            await self.client.users.by_user_id(self.user.email).outlook.master_categories.post(body=ms_cat)
        except Exception as e:
            raise RuntimeError(f"Failed to add category for user {self.user.id} via MS Graph: {e}") from e
    ...
    async def _list_async(self) -> List[Category]:
        ...
        categories = []
        try:
            ms_categories = await self.client.users.by_user_id(self.user.email).outlook.master_categories.get()
            ...
            raise RuntimeError(f"Failed to list categories for user {self.user.id} via MS Graph: {e}") from e
    ...
    async def _update_async(self, category: Category) -> None:
        ...
        try:
            ...
            if category_id:
                ...
                await self.client.users.by_user_id(self.user.email).outlook.master_categories.by_outlook_category_id(category_id).patch(body=ms_cat)
            else:
                raise ValueError(f"Category '{category.name}' not found")
        except Exception as e:
            raise RuntimeError(f"Failed to update category for user {self.user.id} via MS Graph: {e}") from e
    ...
    async def _delete_async(self, category_id: str) -> None:
        ...
        try:
            await self.client.users.by_user_id(self.user.email).outlook.master_categories.by_outlook_category_id(category_id).delete()
        except Exception as e:
            raise RuntimeError(f"Failed to delete category {category_id} for user {self.user.id} via MS Graph: {e}") from e
    ...
    async def _get_by_name_async(self, name: str) -> Optional[Category]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to get category '{name}' for user {self.user.id} via MS Graph: {e}") from e

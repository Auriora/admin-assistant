...
class ScheduledArchiveService:
    ...
    def schedule_from_job_configurations(self, user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary with scheduling results
        """
        return self.background_job_service.schedule_all_job_configurations(user_id=user_id)

    def create_default_job_configurations(self, user_id: Optional[int] = None,
                                        schedule_type: str = 'daily',
                                        schedule_hour: int = 23,
                                        schedule_minute: int = 59,
                                        schedule_day_of_week: Optional[int] = None,
                                        archive_window_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if user_id:
                active_configs = self.archive_config_service.get_active_for_user(user_id)
            ...
            for archive_config in active_configs:
                try:
                    ...
                    job_config = self.job_config_service.create_default_for_archive_config(
                        archive_config_id=archive_config.id,
                        schedule_type=schedule_type,
                    ...
                    logger.info(f"Created JobConfiguration {job_config.id} for ArchiveConfiguration {archive_config.id}")

                except Exception as e:
                    logger.error(f"Failed to create JobConfiguration for ArchiveConfiguration {archive_config.id}: {e}")
        ...
        logger.info(f"JobConfiguration creation completed: {len(results['created_configs'])} created, "
                   f"{len(results['failed_configs'])} failed, {len(results['skipped_configs'])} skipped")

        return results

    def sync_job_configurations_with_archive_configs(self, user_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for job_config in job_configs:
                try:
                    sync_result = self.job_config_service.sync_with_archive_config_status(
                    ...
                    logger.info(f"Synced JobConfiguration {job_config.id}: {sync_result['action']} {sync_result['count']} configs")

                except Exception as e:
                    logger.error(f"Failed to sync JobConfiguration {job_config.id}: {e}")
        ...
        logger.info(f"JobConfiguration sync completed: {len(results['synced_configs'])} synced, "

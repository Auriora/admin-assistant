...
class CategoryProcessingService:
    ...
    def validate_category_format(self, categories: List[str]) -> Dict[str, List[str]]:
        ...
        for category in categories:
            ...
            if customer is not None and billing_type is not None:
                ...
                if ' - ' not in category:
                    result['issues'].append(f"Missing ' - ' separator in category: {category}")
                elif len(category.split(' - ')) != 2:
                    result['issues'].append(f"Too many ' - ' separators in category: {category}")
                elif not category.split(' - ')[0].strip():
                    result['issues'].append(f"Empty customer name in category: {category}")
                elif category.split(' - ')[1].strip().lower() not in self.VALID_BILLING_TYPES and category.lower() not in self.SPECIAL_CATEGORIES:
                    result['issues'].append(f"Invalid billing type in category: {category}")
    ...
    def extract_customer_billing_info(self, appointment: Appointment) -> Dict[str, Any]:
        ...
        if not categories:
            ...
            result['issues'].append("No categories found - treating as personal appointment")
            return result
        ...
        if validation_result['valid']:
            ...
            if len(validation_result['valid']) > 1:
                result['issues'].append(f"Multiple valid categories found, using first: {first_valid}")
    ...
    def _extract_categories_from_appointment(self, appointment: Appointment) -> List[str]:
        """
    ...
    def get_category_statistics(self, appointments: List[Appointment]) -> Dict[str, Any]:
        """
        ...
        for appointment in appointments:
            ...
            if customer_info['is_valid']:
                ...
                if customer_info['billing_type']:
                    ...
                    stats['billing_types'][billing_type] = stats['billing_types'].get(billing_type, 0) + 1
            else:

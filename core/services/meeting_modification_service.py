...
class MeetingModificationService:
    ...
    def process_modifications(self, appointments: List[Appointment]) -> List[Appointment]:
        ...
        for mod_type, modification in modifications:
            original = self.find_original_appointment(modification, regular_appointments)
            if original:
                try:
                    ...
                    if original in processed_appointments:
                        ...
                        modification_log.append(f"Applied {mod_type} to appointment: {getattr(original, 'subject', 'Unknown')}")
                    
                except Exception as e:
                    logger.error(f"Failed to apply {mod_type} modification: {e}")
                    modification_log.append(f"Failed to apply {mod_type}: {e}")
            else:
                logger.warning(f"No original appointment found for {mod_type} modification: {getattr(modification, 'subject', 'Unknown')}")
                modification_log.append(f"Orphaned {mod_type} modification: {getattr(modification, 'subject', 'Unknown')}")
        
        if modification_log:
            logger.info(f"Modification processing completed: {'; '.join(modification_log)}")
    ...
    def merge_extension(self, original: Appointment, extension: Appointment) -> Appointment:
        ...
        extension_duration = getattr(extension, 'end_time', None) - getattr(extension, 'start_time', None)
        if extension_duration:
            ...
            logger.debug(f"Extended appointment '{getattr(original, 'subject', 'Unknown')}' by {extension_duration}")
    ...
    def apply_shortening(self, original: Appointment, shortening: Appointment) -> Appointment:
        ...
        shortening_duration = getattr(shortening, 'end_time', None) - getattr(shortening, 'start_time', None)
        if shortening_duration:
            # Reduce the end time by the shortening duration
            shortened.end_time = getattr(original, 'end_time', None) - shortening_duration

            # Ensure end time doesn't go before start time
            if shortened.end_time <= getattr(original, 'start_time', None):
                shortened.end_time = getattr(original, 'start_time', None) + timedelta(minutes=1)
                logger.warning(f"Shortening would make appointment negative duration, set to 1 minute minimum")

            logger.debug(f"Shortened appointment '{getattr(original, 'subject', 'Unknown')}' by {shortening_duration}")
    ...
    def adjust_start_time(self, original: Appointment, timing_adjustment: Appointment) -> Appointment:
        ...
        mod_type = self.detect_modification_type(getattr(timing_adjustment, 'subject', ''))
        ...
        if mod_type == 'early_start':
            ...
            logger.debug(f"Moved start time earlier for '{getattr(original, 'subject', 'Unknown')}'")

        elif mod_type == 'late_start':
            # For late start, calculate the delay and adjust start time
            delay_duration = getattr(timing_adjustment, 'end_time', None) - getattr(timing_adjustment, 'start_time', None)
            if delay_duration:
                adjusted.start_time = getattr(original, 'start_time', None) + delay_duration

                # Ensure start time doesn't go past end time
                if adjusted.start_time >= getattr(original, 'end_time', None):
                    adjusted.start_time = getattr(original, 'end_time', None) - timedelta(minutes=1)
                    logger.warning(f"Late start would make appointment negative duration, adjusted to 1 minute minimum")

                logger.debug(f"Delayed start time by {delay_duration} for '{getattr(original, 'subject', 'Unknown')}'")
    ...
    def find_original_appointment(self, modification: Appointment, appointments: List[Appointment]) -> Optional[Appointment]:
        ...
        for appt in appointments:
            ...
            mod_type = self.detect_modification_type(getattr(modification, 'subject', ''))
    ...
    def _copy_appointment(self, appointment: Appointment) -> Appointment:
        ...
        for attr in ['user_id', 'start_time', 'end_time', 'subject', 'location_id',
                     'category_id', 'timesheet_id', 'recurrence', 'ms_event_data',
                     'show_as', 'sensitivity', 'location', 'categories', 'importance']:

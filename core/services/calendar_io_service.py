...
def fetch_appointments(
    user, start_date: date, end_date: date, appointment_repo, logger=None
) -> List[Appointment]:
    """
    ...
    try:
        ...
        filtered = [
            ...
            if hasattr(appt, "start_time")
            and hasattr(appt, "end_time")
            and appt.start_time is not None
            and appt.end_time is not None
            and appt.start_time.date() >= start_date
            and appt.end_time.date() <= end_date
        ]
        if logger:
            logger.info(
                f"Fetched {len(filtered)} appointments for {user.email} from {start_date} to {end_date}"
            )
        return filtered
    except Exception as e:
        if logger:
            logger.exception(
                f"Failed to fetch appointments for user {user.email} from {start_date} to {end_date}: {str(e)}"
            )
        ...
        raise CalendarServiceException(
            f"Failed to fetch appointments for user {user.email}"
        ) from e


def store_appointments(
    appointments: List[Appointment], appointment_repo, logger=None
) -> dict:
    """
    ...
    for appt in appointments:
        try:
            ...
            if hasattr(e, "add_note"):
                e.add_note(
                    f"Error in store_appointments for appointment {getattr(appt, 'subject', 'Unknown')}"
                )

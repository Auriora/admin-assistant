...
class OverlapResolutionOrchestrator:
    ...
    def get_open_tasks(self, user_id: int) -> List[Any]:
        ...
        return [
            {
                "task": t,
                "related": self.action_log_service.get_related_entities(
                    getattr(t, "id")
                ),
            }
            for t in tasks
    ...
    def resolve_overlap(
        self, action_log_id: int, resolution_data: dict, user: Any, calendar_id: Any
    ) -> None:
        """
        ...
        with AuditContext(
            ...
            for edit in resolution_data.get("edit", []):
                ...
                if appt:
                    old_values = {
                        k: getattr(appt, k, None) for k in edit.get("fields", {}).keys()
                    }
            ...
            if merge:
                ...
                resolution_actions.append(
                    f"Merged {len(from_ids)} appointments into 1, deleted {deleted_count}"
                )
            ...
            handled_ids = (
                keep_ids
                | {e["id"] for e in resolution_data.get("edit", [])}
                | set(merge.get("from", []) if merge else [])
            )
            ...
            if deleted_unhandled_count > 0:
                resolution_actions.append(
                    f"Deleted {deleted_unhandled_count} unhandled appointments"
                )
    ...
    def get_ai_suggestions(self, action_log_id: int, user_id: int) -> Optional[str]:
        ...
        prompt = self.prompt_service.get_most_relevant_prompt(
    ...
    def start_or_continue_chat(
        self, action_log_id: int, user_id: int, message: str
    ) -> Any:
        """
        ...
        if chat_sessions:
            ...
            session = ChatSession(
                user_id=user_id, messages=[{"sender": "user", "content": message}]
            )
        ...
        self.chat_session_service.append_message(
            getattr(session, "id"), {"sender": "user", "content": message}
        )
